...
# Lexicographic bytewise order == integer order when padded to fixed width 22.
...
def encode_base64(num: int, pad: bool = True) -> str:
    """Encodes a 128-bit integer to fixed-width 22-char Base64 sortable string.
    ...
    return b64_str.rjust(22, BASE64_ALPHABET[0]) if pad else b64_str


# Backward-compat alias (deprecated): old Base62 name maps to Base64 sortable
def encode_base62(num: int, pad: bool = False) -> str:  # pragma: no cover
    """Deprecated alias for encode_base64 (Base62 is removed; use Base64 sortable)."""
    return encode_base64(num, pad=True if pad else True)


def decode_base64(b64_str: str) -> int:
    """Decodes Base64 sortable string (1-22 chars, padded or trimmed) back to 128-bit integer.

    Accepts client-trimmed strings (1-22 chars, leading '0's stripped) and
    DB-padded strings (exactly 22 chars). For DB/storage the string MUST be
    re-padded to 22 with '0' (zero-value) before lexicographic comparison;
    trimmed strings are NOT sortable until re-padded. Case-sensitive.
    """
    if not b64_str:
        raise ValueError("Base64 string cannot be empty.")
    if not (1 <= len(b64_str) <= 22):
        raise ValueError("Base64 Snowflake string must be between 1 and 22 characters long (22 padded for DB, 1-22 trimmed allowed for client display).")
    # Left-pad trimmed client strings to 22 with zero-value '0' for sortable DB form
...
def decode_base62(b62_str: str) -> int:  # pragma: no cover
    """Deprecated alias — now validates as Base64 sortable."""
    # For migration, try Base64 strict first; fall back to error with guidance.
    try:
        return decode_base64(b62_str)
    except ValueError as e:
        raise ValueError(f"Invalid Base62/Base64 string: {e}. Spec now requires Base64 sortable 22-char (alphabet includes '=' and '_').") from e
...
class Snowflake:
    ...
    def to_string(self, pad: bool = True) -> str:
        # pad=True (default) emits fixed 22-char DB-padded form (sortable). pad=False trims leading '0's for client display (1-22, NOT sortable until re-padded).
        return encode_base64(self._raw, pad=pad)
    ...
    def parse(snowflake_val: str | int | bytes) -> Snowflake:
        ...
        if isinstance(snowflake_val, str):
            ...
            if not (1 <= len(snowflake_val) <= 22):
                raise ValueError(
                    "Base64 Snowflake string must be between 1 and 22 characters long (22 padded for DB, trimmed 1-22 allowed for client)."
                )
...
if __name__ == "__main__":
    ...
    def _print_sf(sf: Snowflake) -> None:
        ...
        print(f"  Base64 (sortable 22): {sf.to_string()}")
    ...
    def cmd_interactive() -> None:
        print("Ryuuganime Snowflake ID Generator (Base64 sortable 22-char)\n")
        print("  1) Generate")
        print("  2) Parse")
        choice = input("\nChoice [1]: ").strip() or "1"

        if choice == "2":
            val = input("Enter snowflake (22-char Base64 string or int): ").strip()
            try:
                val = int(val)
            except ValueError:
                pass
    ...
    parser = argparse.ArgumentParser(description="Ryuuganime Snowflake ID tool (Base64 sortable)")
    ...
    p_parse.add_argument("value", help="Base64 22-char string, raw int, or 16-byte hex")
